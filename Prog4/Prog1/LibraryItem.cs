...
// This file also creates a default sort for the library items and sorts them in ascending order by title.
...
namespace LibraryItems
{
    public abstract class LibraryItem : IComparable<LibraryItem>
    {
        ...
        //Precondition: are is a library item
        //Postcondition: The library items have been sorted by type in ascending order
        public int CompareTo(LibraryItem items2)
        {
                if (items2 == null)
                    return 0;

                if (this.Title.CompareTo(items2.Title) != 0)
                    return this.Title.CompareTo(items2.Title);
              else
                    return 0;
        }
    }
